...
def rsa_encrypt(message, receiver_username):
    ...
    cert = x509.load_pem_x509_certificate(open(CA_CLIENT_CERT_DIR+receiver_username+"_cert.pem", 'rb').read(), default_backend())
...
def rsa_decrypt(encrypted_message, receiver_username):
    ...
    rsa_private_key = RSA.importKey(open(CA_CLIENT_KEY_DIR+receiver_username+".key","r").read())
...
# encrypt=rsa_encrypt("hello","cert")
# print(encrypt ,'\n')
# decrypt = rsa_decrypt(encrypt, "cert")
# print(decrypt)